...
public class PVIStream {
    ...
    public static void main(String[] args) {
        ...
        try( Stream<String> lines = Files.lines(path) ){
            ...
            List santaClara = matrix.stream().filter((i) -> i.get(5).contains("Santa Clara")).collect(Collectors.toList()).get(0);

            List miami = matrix.stream().filter((i) -> i.get(5).contains("Miami-Dade")).collect(Collectors.toList()).get(0);
            
            String totaldeath = matrix.stream().filter((i) -> i.get(5).contains("New York"))
